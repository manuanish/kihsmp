var HDProjection = DynmapProjection.extend({
  fromLocationToLatLng: function (location) {
    ...
    return new L.LatLng(
      -(
        ((128 << this.options.tilescale) - lat) /
  ...
  fromLatLngToLocation: function (latlon, y) {
    var ptw = this.options.maptoworld,
      lat =
        (128 << this.options.tilescale) +
        latlon.lat * (1 << this.options.mapzoomout),
...
var HDMapType = DynmapTileLayer.extend({
  ...
  zoomprefix: function (amount) {
    ...
    // amount == 2 -> 'zz_'
    return (
      "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz".substr(0, amount) +
      (amount === 0 ? "" : "_")
    );
  },
});

maptypes.HDMapType = function (options) {
