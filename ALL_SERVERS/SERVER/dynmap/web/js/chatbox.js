componentconstructors["chatbox"] = function (dynmap, configuration) {
  ...
  if (dynmap.getBoolParameterByName("hidechat")) return;
  var chat = $("<div/>").addClass("chat").appendTo(dynmap.options.container);
  var messagelist = $("<div/>").addClass("messagelist").appendTo(chat);
  ...
  if (configuration.scrollback) {
    messagelist.addClass("scrollback").click(function () {
      $(this).hide();
    });
  }
  ...
  if (dynmap.options.allowwebchat) {
    if (dynmap.options.loggedin || !dynmap.options["webchat-requires-login"]) {
      ...
      var login = $("<button/>")
        ...
        .text(dynmap.options["msg-chatrequireslogin"])
        .appendTo(chat);
    }
  }
  ...
  var addrow = function (row) {
    if (configuration.scrollback) {
      ...
      c.slice(0, Math.max(0, c.length - configuration.scrollback)).each(
        function (index, elem) {
          $(elem).remove();
        },
      );
    ...
    if (dynmap.options.joinmessage.length > 0 && playername.length > 0) {
      addrow(
        $("<div/>")
          .addClass("messagerow")
          .append(
            dynmap.options.joinmessage.replace("%playername%", playername),
          ),
      );
    } else if (
      dynmap.options["msg-hiddennamejoin"].length > 0 &&
      playername.length == 0
    ) {
      addrow(
        $("<div/>")
    ...
    if (dynmap.options.quitmessage.length > 0 && playername.length > 0) {
      addrow(
        $("<div/>")
          .addClass("messagerow")
          .append(
            dynmap.options.quitmessage.replace("%playername%", playername),
          ),
      );
    } else if (
      dynmap.options["msg-hiddennamequit"].length > 0 &&
      playername.length == 0
    ) {
      addrow(
        $("<div/>")
  ...
  $(dynmap).bind("chat", function (event, message) {
    ...
    var messageRow = $("<div/>").addClass("messagerow");

    var playerIconContainer = $("<span/>").addClass("messageicon");

    if (
      message.source === "player" &&
      configuration.showplayerfaces &&
      playerAccount
    ) {
