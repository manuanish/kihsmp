...
componentconstructors["markers"] = function (dynmap, configuration) {
  ...
  function loadmarkers(world) {
    removeAllMarkers();
    var url = concatURL(
      dynmap.options.url.markers,
      "_markers_/marker_" + world + ".json",
    );

    $.getJSON(url, function (data) {
      var ts = data.timestamp;
      $.each(data.sets, function (name, markerset) {
        if (markerset.showlabels == undefined)
          markerset.showlabels = configuration.showlabel;
        ...
        if (!ms) {
          ms = {
            ...
            hide: markerset.hide,
            layerprio: markerset.layerprio,
            minzoom: markerset.minzoom || -1,
            maxzoom: markerset.maxzoom || -1,
            showlabels: markerset.showlabels,
            markers: {},
            areas: {},
            lines: {},
            circles: {},
        ...
        $.each(markerset.markers, function (mname, marker) {
          ms.markers[mname] = {
            ...
            desc: marker.desc,
            dim: marker.dim,
            minzoom: marker.minzoom || -1,
            maxzoom: marker.maxzoom || -1,
          };
        ...
        $.each(markerset.areas, function (aname, area) {
          ms.areas[aname] = {
            ...
            minzoom: area.minzoom || -1,
            maxzoom: area.maxzoom || -1,
          };
        ...
        $.each(markerset.lines, function (lname, line) {
          ms.lines[lname] = {
            ...
            minzoom: line.minzoom || -1,
            maxzoom: line.maxzoom || -1,
          };
        ...
        $.each(markerset.circles, function (cname, circle) {
          ms.circles[cname] = {
            ...
            minzoom: circle.minzoom || -1,
            maxzoom: circle.maxzoom || -1,
          };
  ...
  function getPosition(marker) {
    return dynmap
      .getProjection()
      .fromLocationToLatLng({ x: marker.x, y: marker.y, z: marker.z });
  }
    ...
    marker.our_layer = new L.CustomMarker(markerPosition, {
      elementCreator: function () {
        ...
        $(div)
          ...
          .append(
            $("<img/>")
              .addClass("markerIcon" + marker.dim)
              .attr({ src: url }),
          );
        if (marker.label != "")
          $(div).append(
  ...
  function createMarkerSet(set, ts) {
    ...
    set.timestamp = ts;
    if (!set.hide) dynmap.map.addLayer(set.layergroup);
  ...
  function createArea(set, area, ts) {
    ...
    if (area.x.length == 2) {
      ...
      if (area.ytop == area.ybottom) {
        area.our_layer = create2DBoxLayer(
          area.x[0],
          area.x[1],
          area.ytop,
          area.ybottom,
        ...
        area.our_layer = create3DBoxLayer(
          area.x[0],
          area.x[1],
          area.ytop,
          area.ybottom,
      ...
      if (area.ytop == area.ybottom) {
        area.our_layer = create2DOutlineLayer(
          area.x,
          area.ytop,
          area.ybottom,
          area.z,
        ...
        area.our_layer = create3DOutlineLayer(
          area.x,
          area.ytop,
          area.ybottom,
          area.z,
  ...
  latlng = function (x, y, z) {
    return dynmap
      .getProjection()
      .fromLocationToLatLng(new Location(undefined, x, y, z));
  };
  ...
  function create2DOutlineLayer(xarray, maxy, miny, zarray, style) {
    ...
    } else return new L.Polygon(llist, style);
  }
  ...
  $(dynmap).bind("component.markers", function (event, msg) {
    if (msg.msg == "markerupdated") {
      ...
      if (!dynmapmarkersets[msg.id]) {
        dynmapmarkersets[msg.id] = {
          id: msg.id,
          label: msg.label,
          layerprio: msg.layerprio,
          minzoom: msg.minzoom,
          maxzoom: msg.maxzoom,
        ...
        if (
          ...
          dynmap.addToLayerSelector(
            dynmapmarkersets[msg.id].layergroup,
            dynmapmarkersets[msg.id].label,
