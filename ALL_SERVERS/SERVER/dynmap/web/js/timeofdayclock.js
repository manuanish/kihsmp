componentconstructors["timeofdayclock"] = function (dynmap, configuration) {
  ...
  if (configuration.showdigitalclock) {
    ...
    var setTime = function (servertime) {
      ...
      if (servertime >= 0) {
        ...
        clock.removeClass("day night").text("");
      }
  ...
  if (configuration.showweather) {
    var weather = $("<div/>").addClass("weather").appendTo(element);
    ...
    var setWeather = function (hasStorm, isThundering, time) {
      ...
      weather
        .removeClass(
          "stormy_day stormy_night sunny_day sunny_night thunder_day thunder_night",
        )
  ...
  $(dynmap).bind("worldupdated", function (event, update) {
    ...
    if (time >= 0) {
      sun.css(
        "background-position",
        -50 * Math.cos(sunangle) + "px " + -50 * Math.sin(sunangle) + "px",
      );
      moon.css(
        "background-position",
        -50 * Math.cos(moonangle) + "px " + -50 * Math.sin(moonangle) + "px",
      );
    } else {
