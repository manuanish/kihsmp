componentconstructors["logo"] = function (dynmap, configuration) {
  var Logo = L.Control.extend({
    ...
    getPosition: function () {
      if (configuration.position == "top-left") return "topleft";
      else if (configuration.position == "top-right") return "topright";
      else if (configuration.position == "bottom-left") return "bottomleft";
    ...
    _update: function () {
      ...
      if (configuration.logourl) {
        $(c).append(
          $("<img/>")
            .attr("src", configuration.logourl)
            .attr("alt", configuration.text)
            .appendTo(c)[0],
        );
      } else {
