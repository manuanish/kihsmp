...
componentconstructors["testcomponent"] = function (dynmap, configuration) {
  ...
  $(dynmap).bind("zoomchanged", function () {
    console.log("zoomchanged");
  });
  $(dynmap).bind("worldupdating", function () {
    console.log("worldupdating");
  });
  $(dynmap).bind("worldupdate", function () {
    console.log("worldupdate");
  });
  $(dynmap).bind("worldupdated", function () {
    console.log("worldupdated");
  });
  $(dynmap).bind("worldupdatefailed", function () {
  ...
  $(dynmap).bind("playerupdated", function () {
...
function DynMap(options) {
  var me = this;
  if (me.checkForSavedURL()) return;
  me.options = options;
  $.getJSON(
    me.formatUrl("configuration", { timestamp: me.lasttimestamp }),
...
DynMap.prototype = {
  ...
  configure: function (configuration) {
    ...
    if (urlarg == "") urlarg = me.options.defaultworld || "";
    if (urlarg != "") {
      me.defaultworld = me.worlds[urlarg] || me.defaultworld;
    }
    urlarg = me.getParameterByName("mapname");
    if (urlarg != "") {
      me.defaultworld.defaultmap =
        me.defaultworld.maps[urlarg] || me.defaultworld.defaultmap;
    }
    urlarg = me.getIntParameterByName("x");
    if (urlarg != null) me.defaultworld.center.x = urlarg;
    urlarg = me.getIntParameterByName("y");
    if (urlarg != null) me.defaultworld.center.y = urlarg;
    urlarg = me.getIntParameterByName("z");
    if (urlarg != null) me.defaultworld.center.z = urlarg;
  ...
  initialize: function () {
    ...
    // class of "map".
    var mapContainer;
    (mapContainer = $("<div/>")).addClass("map").appendTo(container);

    // Set the title if the options specify one.
    if (me.options.title) document.title = me.options.title;

    // Try to set the default zoom level based on the URL parameter.
    var urlzoom = me.getIntParameterByName("zoom");
    if (urlzoom != null) me.options.defaultzoom = urlzoom;
    ...
    if (showlayerctl != "") me.options.showlayercontrol = showlayerctl;
    ...
    if (initfollowplayer != "") me.initfollow = initfollowplayer;
    ...
    var pincls = "pinned";
    if (me.options.sidebaropened == "false") pincls = "";
    sidebar = me.sidebar = $("<div/>").addClass("sidebar " + pincls);

    panel = me.sidebarPanel = $("<div/>").addClass("panel").appendTo(sidebar);
    ...
    if (!nopanel) sidebar.appendTo(container);

    var worldsSection = SidebarUtils.createListSection(
      me.options["msg-maptypes"],
    );
    ...
    $.each(me.worlds, function (index, world) {
      ...
      world.element = $("<li/>")
        ...
        .text(world.title)
        .append((maplist = $("<ul/>").addClass("maplist sublist")))
        .data("world", world);
    ...
    $.each(me.worlds, function (index, world) {
      ...
      $.each(world.maps, function (mapindex, map) {
        ...
        if (worldName.endsWith("_nether") || worldName == "DIM-1") {
          ...
          worldName = "world";
          mapName = ["surface", "flat", "biome", "cave"].includes(mapindex)
            ? mapindex
            : "flat";
        }
        map.element = $("<li/>")
            ...
            $("<a/>")
              ...
              .css({
                backgroundImage:
                  "url(" +
                  (map.options.icon ||
                    "images/block_" + worldName + "_" + mapName + ".png") +
                  ")",
              })
    ...
    var playersSection = SidebarUtils.createListSection(
      me.options["msg-players"],
    );
    ...
    if (!me.nogui && !me.nocompass) {
      var compass = $("<div/>").addClass("compass");
      if (L.Browser.mobile) compass.addClass("mobilecompass");
    ...
    if (
      dynmapversion != me.options.coreversion &&
      dynmapversion.indexOf("-Dev") < 0
    ) {
      // Disable on dev builds
      me.alertbox
        .text(
          "Web files are not matched with plugin version: All files need to be same version (" +
            me.options.dynmapversion +
            ") - try refreshing browser cache (shift-reload)",
        )
        .show();
      return;
    }
    ...
    if (me.nogui) {
      ...
      setTimeout(function () {
        $.each(configset, function (type, configlist) {
          if (tobeloaded[type]) {
            me.alertbox.text("Error loading js/" + type + ".js").show();
          }
        });
        if (componentstoload > 0)
          setTimeout(function () {
  ...
  updateSidebarHeight: function () {
    ...
    $.each(me.sidebarSections, function (i, section) {
      ...
      var sectionPadding =
        section.section.outerHeight(true) - section.section.height();
    ...
    if (sectionsHeight > sidebarHeight && sidebarHeight > minSectionsHeight) {
      ...
      $.each(me.sidebarSections, function (i, section) {
        ...
        var proportionalMissingHeight =
          missingHeight * (contentHeight / resizeableContentHeight);
  ...
  selectMapAndPan: function (map, location, completed) {
    ...
    if (me.maptype) {
      compass
        .removeClass("compass_" + me.maptype.options.compassview)
        .removeClass("compass_" + me.maptype.options.name);
    }
    compass
      .addClass("compass_" + map.options.compassview)
      .addClass("compass_" + map.options.name);
    var worldChanged = me.world !== map.options.world;
    var projectionChanged =
      (me.maptype && me.maptype.getProjection()) !== (map && map.projection);
    ...
    if (worldChanged) {
      // World changed - purge URL cache (tile updates unreported for other worlds)
      me.registeredTiles = [];
      me.inittime = new Date().getTime();
    }

    if (worldChanged && me.world) {
      me.world.lastcenter = me.maptype
        .getProjection()
        .fromLatLngToLocation(me.map.getCenter(), 64);
    }
    ...
    if (projectionChanged || worldChanged || location) {
      ...
      if (location) {
        ...
        var centerLocation;
        if (mapWorld.lastcenter) centerLocation = mapWorld.lastcenter;
        else centerLocation = $.extend({ x: 0, y: 64, z: 0 }, mapWorld.center);
        ...
        else centerPoint = me.map.getCenter();
      }
  ...
  update: function () {
    ...
    $.getJSON(
      ...
      function (update) {
        ...
        if (me.lasttimestamp == update.timestamp) {
          // Same as last update?
          setTimeout(function () {
            me.update();
          }, me.options.updaterate);
  ...
  getTileUrl: function (tileName) {
    ...
    if (tile == null) {
      ...
      tile = this.registeredTiles[tileName] =
        url + escape(me.world.name + "/" + tileName);
    }
    return tile;
  },
  addPlayer: function (update) {
    ...
    var menuitem = (player.menuitem = $("<li/>")
      ...
      .click(function (e) {
        ...
        if (player.location.world) me.panToLocation(player.location);
      }));
    player.menuname.data("sort", player.sort);
    // Inject into playerlist alphabetically
    var firstNodeAfter = me.playerlist
      .children()
      .filter(function () {
        var itm = $("a", this);
  ...
  updatePlayer: function (player, update) {
    var me = this;
    var location = (player.location = new Location(
      me.worlds[update.world],
      parseFloat(update.x),
      parseFloat(update.y),
      parseFloat(update.z),
  ...
  followPlayer: function (player) {
    ...
    if (player) {
      if (!player.location.world) return;
      ...
      me.panToLocation(player.location, function () {
        if (me.options.followmap && me.world) {
          ...
          if (pmap) me.selectMapAndPan(pmap);
        }
        if (me.options.followzoom) me.map.setZoom(me.options.followzoom);
      });
    }
  ...
  addToLayerSelector: function (layer, name, priority) {
    ...
    if (me.options.showlayercontrol != "false") {
      ...
      for (i = 0; i < me.layersetlist.length; i++) {
        me.layercontrol.addOverlay(
          me.layersetlist[i].layer,
          me.layersetlist[i].name,
          i,
        );
      }
    }
  ...
  initLogin: function () {
    var me = this;
    if (!me.options["login-enabled"]) return;
